...
def scan():
    ...
    try:
        r = requests.get(
            f"{CG_BASE}/futures/coins-markets",
            headers={"CG-API-KEY": key},
            timeout=10
        )
        data = r.json()
        print(f"CoinGlass response: {data.get('code')} {data.get('msg')}")
        out = []
        for item in data.get("data", []):
            sym = item.get("symbol","") + "USDT"
            if sym in COINS:
                out.append({
                    "symbol": sym,
                    "price": str(item.get("current_price", 0)),
                    "change": str(item.get("price_change_percent_24h", 0)),
                    "volume": str(item.get("volume_usd_24h", 0)),
...
def funding():
    ...
    try:
        r = requests.get(
            f"{CG_BASE}/futures/funding-rate/exchange-list?symbol={sym}",
            headers={"CG-API-KEY": key},
            timeout=10
        )
